# @GEMINI.md — Authoritative Engineering, Architecture & Defect Prevention Directives

> **Unified IT Management System (UIMS)**  
> This document is the authoritative operational guide and invariant contract for Google Gemini AI agents, Antigravity CLI agents, automated tooling, and contributors operating on the UIMS codebase. It is strictly harmonized with `AGENTS.md` and codifies monorepo boundaries, zero-defect engineering directives, type safety standards, security invariants, database performance rules, Ant Design v6 specifications, and the 5-gate verification cycle.
...
- **Monorepo Structure**:
  ...
  - `packages/shared-utils`: Common string, date, currency, byte, and network validation utilities.
...
## 2. Agent Behavioral Contracts & Integrity Mandate
...
3. **Surgical Changes**:
   ...
   - Clean up your own mess: remove imports, variables, and functions made unused by your changes.
...
5. **Absolute Integrity Mandate**:
   - All implementations must be genuine.
   - Never hardcode test results, expected outputs, or verification strings in source code.
   - Never create dummy or facade implementations that produce correct-looking outputs without genuine logic.
   - Never circumvent intended tasks by delegating core work to external tools or pre-built solutions when building from scratch is required.
   - Maintain real state and produce real behavior across all modules and tests.
...
## 4. Zero Silent Catch Policy & Defect Prevention Directives

- **Prohibition of Silent Catch Blocks**:
...
- **Backend Service Error Handling**:
  - Every error caught must be either:
    ...
    3. Gracefully handled with an explicit, traceable fallback response.
...
- **Frontend Component Error Handling**:
  - Every error caught in UI components, hooks, or event handlers must surface actionable user-facing feedback via `App.useApp().message.error(...)` / `notification.error(...)` or propagate to the nearest `ErrorBoundary`.
  - Raw `console.error(...)` and `console.log(...)` calls are strictly forbidden in production code. Centralized error reporting and user feedback are mandatory.
...
- **Principle of Least Privilege & Fail-Closed Access**:
...
## 6. Network, CORS & Controller Architecture
...
## 7. Database Access & Performance Directives
...
## 8. Structured Logging Standards & Telemetry
...
## 10. Ant Design v6+ UI/UX Directives & Zero Anti-Pattern Mandate

> **Official Ant Design Reference Specification**:  
> All UI modifications, component authoring, form layouts, tables, drawers, modals, and styling MUST strictly comply with the authoritative Ant Design LLM specification located at [`docs/ant-design-llms-full.txt`](docs/ant-design-llms-full.txt). Zero anti-patterns permitted.
...
## 11. Enterprise English & UI/UX Copy Standards
...
- **Strict Zero-Downgrade Policy**:
  ...
  - React 19 (`^19.2.8`), Vite 8 (`^8.2.2`), Ant Design 6 (`^6.6.3`), NestJS 11 (`^11.2.3`), and Prisma 7 (`^7.10.0`) must be maintained at their latest stable releases without downgrades.
...
## 13. System Startup, Background Daemon & Stack Management Directives

Whenever starting, stopping, checking, or restarting the system (e.g., "start system", "run app", "restart", "status"):
...
## 14. Docker & Infrastructure Port Topology Matrix
...
## 15. Single Public HTTPS Port Architecture (Port 5679)
...
Every change must satisfy the full 5-gate verification cycle prior to merging or pushing:
